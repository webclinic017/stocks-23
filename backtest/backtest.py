...
class Strategy(bt.Strategy):
	...
	def __init__(self):
		...
		# trend filter
		self.adx = bt.indicators.AverageDirectionalMovementIndex(self.datas[0], period=14)
		...
		self.plusdi = bt.indicators.PlusDirectionalIndicator(self.datas[0], period=14)
		self.minusdi = bt.indicators.MinusDirectionalIndicator(self.datas[0], period=14)

		# non trending indicator
		self.bolls= bt.indicators.BollingerBands(self.datas[0],period=20, devfactor=2)
		self.order = None
		self.buyprice = None
		self.buycomm = None
	...
	def buy_indicator(self):
		if (self.adx[0] >30 or self.adx[0] > self.adx[-1]):
			# trending
			if (self.sma4[0] > self.sma9[0] and self.plusdi[0] > self.minusdi[0]):
				return True
		else:
			# non trending
			if (self.bolls[0] > self.bolls.bot[0] and self.bolls[-1] < self.bolls.bot[-1]):
				return True				
		return False
	...
	def sell_indicator(self):
		if (self.adx[0] >30 or self.adx[0] > self.adx[-1]):
			...
			if (self.bolls[0] <self.bolls.top[0] and self.bolls[-1] > self.bolls.top[-1]):
				return True
		return False
	def next(self):
		if self.order:
